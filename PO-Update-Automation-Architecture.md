...
- **New finding, blocks Phase 2's PO-number-to-internal-id resolution:** the least-privilege "PO Update" role can read individual records directly by internal id (`GET /purchaseOrder/{id}`) but **cannot perform collection-level GET/search queries** — `GET /purchaseOrder?limit=1`, `GET /inventoryItem?limit=1`, and `GET /vendor?limit=1` all returned `400 USER_ERROR` for this role, despite `Lists > Items` and `Lists > Vendors` being granted at View level. Record-level View apparently doesn't confer collection/search access — that appears to need a separate permission. This matters because vendor packing slips carry human PO numbers (e.g. "1662"), not NetSuite internal ids (e.g. "8489541") — resolving one to the other is exactly what a collection/search query is for, and the matcher depends on this from Phase 2 onward. **Needs a NetSuite admin conversation**: which specific permission grants REST collection/search access, and whether it can be added without granting broad search rights across the account. Add this to the same conversation as the OAuth Access Tokens permission and the service-account license question above.
