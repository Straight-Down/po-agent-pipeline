...
- **NetSuite does NOT derive `expectedReceiptDate` from the override pair — the tool must write all three date fields (2026-08-12, sandbox-tested).** Worth stating because the opposite is the natural assumption: the two custom fields are *called* "Override Expected Receipt" and "Updated Receipt Date", which reads like a mechanism that feeds the standard field.

  **The test, on PO 8489541 line 18:** PATCHed **only** `custcol_override_expected_receipt = true` and `custcol_sd_updatedreceiptdate = '2026-12-01'`, deliberately omitting `expectedReceiptDate`. HTTP 204, both sent fields took. Read back: **`expectedReceiptDate` was unchanged at `'2026-07-15'`** — it did not move to the new date, and it did not move anywhere else. Reverted all three explicitly and confirmed the line identical to its original state on all three fields.

  **Conclusion: write `expectedReceiptDate`, `custcol_override_expected_receipt = true` and `custcol_sd_updatedreceiptdate` together, with the same value.** Setting only the override pair would leave the field NetSuite actually schedules against stale — the flag and the updated date would say December while the PO still expected July. The current write payload already does this; the finding confirms it rather than changing it.

  **Caveat, and it is a real one: this tests the REST save path only.** Whether anything fires **downstream** of a *UI* save that a REST PATCH skips — a client-side script on the PO form, a workflow, a RepSpark feed, a notification — is **untested**. Note *why* it is untested rather than merely unmeasured: every live test so far has been write-verify-**revert**, and reverting inside a few seconds is precisely what prevents a downstream consumer from ever observing the change. The write path is proven; its side effects are not. **Added as a Phase 4 pre-cutover verification item** (build plan Phase 4), because the first shipment where a change is *left in place* is the first time this can be observed at all.
- **`(PO, style, colour, size)` is NOT unique per NetSuite PO line (2026-08-24).** The matching key the whole design rests on can resolve to several lines. **64 of 1,659 POs (3.9%)** carry duplicate-key lines, 451 surplus lines in total.

  **They are created during receiving, not at PO entry** — **0 of 89** Pending Receipt POs have them, versus 4 of 17 Partially Received. The 0/89 is the well-powered half (treat the 24% as directional, n=17), and the consequence is that this pipeline's exposure is **much higher than 3.9%**: a second packing slip against a partially-received PO is the exact case the tool exists for. On the live population — the 5 POs a packing slip could still act on — **25 duplicate groups: 24 with exactly one open line, 1 with two**, and every live group had at least one open line.

  **No single field discriminates the pair, and the field data cannot yield a rule.** Across 435 pairs: **date-driven (233)** and **non-date (202)**, the latter differing on `rate`, `description` or the RepSpark flag. `rate` differing in **21%** means some pairs are **separate commercial transactions at different prices**, not a split of one order — so "the second line" is not one phenomenon. `custcol_sd_fg_excluderepspark` differs in only **25.5%**: it was **not** the discriminator, and it differs because a human sets it case by case.

  **The rule as built** (`matcher._resolve_target_line`, change 5): gather **all** matching lines → filter to `isOpen` → **exactly one** open line → target it; **zero** → flagged, no write; **two or more** → `NEEDS_RESOLUTION` carrying each candidate's quantity, received, billed, dates and rate. `LineAmbiguous` makes the refusal structural rather than advisory. **No tiebreaker, ever:** `quantityReceived` looks like it would settle the one live case (50 units received 0 versus 200 received 100) but that is **n=1**, and a wrong automatic pick fails **silently** — wrong line updated, right line stale, nothing to notice. Surface the figures; never branch on them.

  **Two problems share this symptom and need opposite fixes.** Extraction-side duplicates (one key across carton rows in one document) must be **summed** — that is change 2, `extraction_schema.aggregate_lines`. NetSuite-side duplicates (one key, several PO lines) must **never** be summed. Applying either fix to the other's case is a silent data error in a different direction. Full evidence in RUNBOOK §6 item 10.
...
- **A shipment is not 1:1 with a PO.** One Inprotex sheet interleaves **six** POs (1640/1645/1650/1662/1667/1704); the Symmetry set spans two. So `shipments`/`proposed_changes` (§5) must model one email spanning many POs; the Phase 3 **approval unit** must be chosen deliberately — **RESOLVED 2026-09-16: per PO**, because per shipment would fire writes across these six unrelated orders from one click; and write-back needs **partial-failure semantics** — **RESOLVED 2026-09-16: successes stand, failures are flagged with the reason and retried individually, no rollback and no all-or-nothing.** Both in `PO-Update-Automation-Phase3-Requirements.md`.

### 6.3 Scope boundaries — what this tool never does (decided)

Four boundaries, settled rather than provisional. Each is asserted by a test, not only written down here, because a boundary that exists only in prose erodes the first time it is inconvenient.
...
- **Re-shipment / split-shipment and over-shipment semantics. RESOLVED for the over-ship case, now built. Still open for true multi-batch shipments.** Paula: **"the packing list is the correct document to use, the PO will have the original quantity that I ordered and then when production is complete, there are always extra units that we accept."** Implemented: replace semantics, and over-shipment produces a plain `PENDING_REVIEW` with no attention flag — verified against a 500-shipped-vs-100-ordered case. **RESOLVED 2026-09-16, and it was not low urgency.** Paula: **"The vendor's packing slip only shows the new shipment's quantities."** So a second batch ACCUMULATES. This sat here as low urgency on the reasoning that it fires rarely, but what it produced when it fired was a line written **down** from 128 to 100 — 28 units lost, silently. Built 2026-09-16: `matcher._accumulated_quantity`, `ingest._line_history`, migration 0007. **The arithmetic base is this tool's own audit trail, never NetSuite's current quantity** — that is a field this tool writes, and reading it back would let past output decide future output (RUNBOOK §8 lesson 13). NetSuite's value is a consistency check instead: where the two disagree, nothing is proposed and both numbers go to Paula.
...
- ~~Who else, besides Paula, should be able to approve changes (e.g., backup approver when she's out)?~~ **RESOLVED 2026-09-16 — Paula only. No delegation, no backup. Work queues until she returns.** Recorded as her explicit choice rather than an omission, because an absent answer invites a future reader to add a backup approver as an obvious convenience. **The operational consequence, stated plainly: the pipeline has a single approver and no path around her** — while she is away nothing reaches NetSuite, however routine. That is by design, and it is a real single point of failure on a business process.
...
- ~~Confirmed this session: the Outlook/M365 connector only sees the mailbox of whoever authorized it (Kiko), not Paula's mailbox~~ ~~**Resolved 2026-08-10 — Paula's answer: "It can have direct access to my inbox."** Build the Graph API app registration with app-only `Mail.Read` permission scoped directly to her mailbox — no shared mailbox, no forwarding rule needed.~~ **SUPERSEDED 2026-08-24 — the target is a new shared mailbox, and Paula's inbox was explicitly rejected.** Reason: **Graph's `Mail.Read` application permission is mailbox-level, not folder-level.** There is no way to grant an app-only application access to one folder, so pointing it at her account would expose her entire inbox to the service — her willingness to allow it does not make it the right scope. Build instead: a **new shared mailbox (`shipments@`)** with app-only `Mail.Read` scoped to that mailbox alone via **RBAC for Applications**. **Two admin roles are involved**, which makes this a scheduling dependency rather than a task: **Entra ID** admin (Application Administrator or Global Administrator) for the app registration and tenant-wide consent, and **Exchange** admin for the shared mailbox and the RBAC scope. Confirm both, by name, before Phase 2 starts. See RUNBOOK §6 item 11.
