...
5. Move from sandbox to production NetSuite Integration record/role (mirroring Phase 1 setup). ~~**Least-privilege check while doing it:** test whether **View** suffices for `Reports > SuiteAnalytics Workbook` and downgrade if it does.~~ **That check is CLOSED, 2026-09-09 — no action at cutover.** `Reports > SuiteAnalytics Workbook` offers **only `Edit`** in the role editor's level dropdown; `View` is not an option, so `Edit` is the minimum NetSuite permits and not a compromise to unwind. Oracle's documentation says *"set the access to Edit"* rather than describing a range. Grant it at `Edit` on the production role and move on.
6. **Lock `audit_log` down to append-only at the database, not by convention.** The application never updates or deletes those rows, but nothing stops it — and an audit trail that *can* be rewritten is worth less than one that cannot. Create a database role for the application with `INSERT` and `SELECT` on `audit_log` and **no `UPDATE` or `DELETE`**, and connect as it rather than as the schema owner. Azure SQL: a contained user with explicit `GRANT`/`DENY`. Worth doing at cutover rather than later, because retrofitting it means finding out whether anything had quietly come to depend on the ability. (Recorded in schema rationale §7; this is the phase that has to act on it.)
7. Basic monitoring/alerting (e.g., "intake job hasn't run in X hours," "NetSuite write failed") so silent failures don't sit unnoticed.
8. Short handoff doc for Paula (how to use the review/approval step) and for whoever maintains it after Kiko.
