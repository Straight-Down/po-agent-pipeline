...
30. **NEW 2026-09-16 — Paula ruled on the four questions Phase 3 was blocked on, and one of them turned out to be a live data-loss bug rather than a design question.** Recorded together because the fourth is the reason the other three matter less than it does.
    ...
    **NetSuite's value is a consistency check, and a disagreement is a full stop, not a reconciliation:**
    ...
    | No history, goods already received | `PRE_EXISTING_RECEIPT` — **not a dispute**; confirm the total once, then it retires |
    ...
    **THE DAY-ONE CASE IS ITS OWN BASIS, NOT A DISPUTE.** `PRE_EXISTING_RECEIPT` covers "no history, and the line already carries receipts from before this tool existed". Nothing contradicts anything there — there is simply nothing to add to — so it asks a question (*"this line had N units received before the tool started tracking it; confirm the total"*) rather than raising an alarm. `DISPUTED` is reserved for the two cases where our record and NetSuite genuinely **contradict**: we wrote one number and the line holds another, or the line moved since we last looked. Migration 0008 splits them.

    **Why they cannot share a label.** `PRE_EXISTING_RECEIPT` is guaranteed to arrive in a batch on first contact; `DISPUTED` must never be routine. One label for both means a predictable wave of expected rows wearing the word that is supposed to mean "someone changed something behind us" — and a reviewer who sees that word on hundreds of unremarkable lines learns within a week to skim it. The case she then skims past is the alarming one. **This is the same failure the extraction-confidence flag already demonstrated on this project** (§7): `needs_review` fired on 19 of 29 correctly matched lines, and a signal that fires on expected conditions carries no information however honest each individual firing is. It is worth recognising as a class — *a signal is worth exactly as much as the cases it does NOT fire on.*

    **The wave, measured rather than guessed — 426 open PO lines across 33 POs** (sandbox, 2026-09-16, `scripts/estimate_pre_existing_receipts.py`). That is a ceiling: a line only asks once a slip actually touches it, so the real wave is the part of those 426 that ships in the coming weeks, and each one asks exactly once before retiring itself. **Re-run against production before cutover** — sandbox receipt state can diverge and nothing would say so (§8 lesson 16).

    **Getting that number right took three attempts, and the first two were plausible.** `transactionLine` in SuiteQL exposes `isclosed` but **no `isopen`**, so the obvious query filters `isclosed = 'F'` and returns **23,981** lines — **56x** the truth. It is the `isClosed` ≠ `not isOpen` trap from §6 item 10 resurfacing somewhere new: on a Fully Billed PO every line has `isClosed = F` *and* `isOpen = F`, so that filter counts every settled line ever received against. A second attempt sampled the 25 most recent POs and got **0**, which is true and useless — selecting recent POs selects for POs not yet received against, the same sampling trap as the duplicate-key survey (§8). A third narrowed to not-fully-received lines and got **41**, still short, because a line can be fully received and still open while its PO waits on billing. **0, 41, 426: where you draw the net IS the result.** The script narrows with SuiteQL and then confirms every candidate through the same `POLine.is_open` the matcher actually branches on, rather than trusting a proxy for it.

    **DO NOT BACKFILL `write_attempts` TO SUPPRESS THE WAVE.** It is the obvious optimisation and it must not be done. Writing "this tool wrote N" for a quantity this tool did not write is **fabricating an audit record** — and not a harmless one, because that fabricated number becomes the base every later accumulation on that line is computed from. One wrong invented base is wrong forever, compounding with each shipment, and indistinguishable afterwards from a real write. It would also destroy the only evidence that a human ever confirmed the total. The wave is 426 one-time questions; a bad backfill is a permanent, silent, unfalsifiable error in the arithmetic. Accept the questions.
