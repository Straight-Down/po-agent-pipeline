...
20. **NEW 2026-09-02 — the Tainan file's two sheets, `ACT` and `REV`, are two versions of ONE shipment, not two styles. Both are read, and their quantities are being summed.** Needs Paula's confirmation of which governs.

    The evidence, from NetSuite rather than from reading the tabs:
    - `ACT` is style **`50144`**, two pages (`NEW INDIGO`, `SILVER`), totalling **865** pcs against `ORD 800`.
    - `REV` is style **`50144-2`**, the same PO, the same two colours, totalling **~860** pcs against the same 800.
    - **PO 1725 contains one style: `50144`.** `50144-2` does not exist on it. So `REV` is not a second garment.
    - `ACT`'s `ORDER` rows reproduce all 28 NetSuite lines **exactly** — NIN `30-32`=16, `32-32`=85 … SLV `42-34`=2, summing to 800. That is a full match on both colours and both inseams.
    - `REV` carries a fractional row (`17.28`, `91.80`, `90.72` …) that is an 8%-uplift *calculation*, a pack-ratio note (`30-32=34`, `34-36=32`, `38-40=30`), and a `KETERANGAN` production block (CUTTING / SEWING / WASHING / PACKING). `ACT` has none of these. That reads as a planning worksheet against an actual packing list — but it is **inference, not fact**, and the filename (`Correction on Aug.10 from Aug.07`) does not say which tab is the correction. Both tabs are dated 2026-08-10.

    Current behaviour, measured 2026-09-02: both sheets extract, giving **28 lines of style `50144` (from `ACT`) and 28 of `50144-2` (from `REV`)**. Because the style strings differ they do **not** merge — the 28 `ACT` lines match PO 1725, and all 28 `REV` lines match nothing and flag with *"no NetSuite line on PO 1725 matches 50144-2/…"*. So the shipment is **not** silently double-counted, and the failure mode is a visible flag rather than a wrong quantity. But the headline total (1725 units) is the sum of two versions of an 865-piece shipment and means nothing on its own — read the per-style split, not the total. The extractor flags the conflict itself: *"Two sheets present with conflicting quantities … Reviewer should confirm which revision governs"*, and it found real data-entry errors in `REV` (row 17: `GW 1.15`, `NW 0.00` for 27 pcs; row 85: carton range reads `'18-'`).

    One more piece of evidence pointing at `ACT`, from the same run: the `ACT` lines came back **`medium`** confidence with a stated reason, while the `REV` lines came back **`high`**. That is not a vote for `REV` — `REV`'s lines are confidently read and confidently unmatchable, which is precisely the shape of correctly extracting the wrong document.

    **Not fixed unilaterally**, because "which revision governs" is Paula's call in the same class as the three business-logic questions in the architecture doc §6.1, and the wrong choice writes wrong quantities. The safe interim state — matching `ACT`, flagging `REV` — is what the code now does by construction.
21. **NEW 2026-09-02 — the empty-size line is a vendor data-entry error, not a composition or a MIXED-carton problem.** It comes from `REV!R85`, where `CTN NO` reads `'18-'` — a truncated carton range, with cartons apparently skipped — and the extractor emitted a quantity-0, low-confidence placeholder rather than dropping the anomaly. **Unrelated to the `MIXED` rows** (`H19`, `H28`, `H29` on `ACT`), which are ordinary multi-size cartons that spread their quantities across several waist columns and read correctly.

    Behaviour is right as-is and is now pinned by a test: a blank size is not a size, so it is never used as a merge key, never matched, and stays flagged. Worth noting only because it is on the `REV` sheet, alongside that sheet's other data-entry errors, which is weak additional support for `ACT` being the authoritative tab in item 20.
